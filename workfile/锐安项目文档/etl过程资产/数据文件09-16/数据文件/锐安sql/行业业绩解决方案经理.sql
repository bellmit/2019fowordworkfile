...
SELECT z_quarter
      ...
      ,SUM(actual) actual
      ,SUM(target) target
  FROM (SELECT to_char(a.recorddate,'yyyy') ||'Q' ||to_char(a.recorddate,'q')  z_quarter
              ,a.post_code
              ,SUM(a.jlyj) actual
              ,0 target
          FROM ods_fi_ind_perform a
         WHERE a.jlyj is not null
         GROUP BY to_char(a.recorddate,'yyyy') ||'Q' ||to_char(a.recorddate,'q') 
                 ,a.post_code
        UNION ALL
        SELECT a.z_quarter
              ,a.post_id
...
SELECT w1.z_quarter
 ...
 on t.post_code = w1.post_code and substr(w1.z_quarter,1,4)=substr(t.z_quarter,1,4)
）  
,
...
SELECT w2.z_quarter
 ...
 on t.post_code = w2.post_code and substr(w2.z_quarter,1,4)= substr(t.z_quarter,1,4)
)
...
SELECT w3.z_quarter
 ...
 on  t.post_code = w3.post_code and substr(w3.z_quarter,1,4)= substr(t.z_quarter,1,4)
 ) 
SELECT w4.z_quarter
