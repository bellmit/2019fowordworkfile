...
select w1.group_id
 ...
 on t.first_dept_id = w1.first_dept_id 
 left join (select * from b where b.z_quarter =1)b
 on w1.first_dept_id=b.jf_deptid
） 
,
...
SELECT w2.group_id
 ...
 on t.first_dept_id = w2.first_dept_id 
 left join (select * from b where b.z_quarter =2)b
 on w2.first_dept_id=b.jf_deptid
)
...
SELECT w3.group_id
  ...
  on t.first_dept_id = w3.first_dept_id 
 left join (select * from b where b.z_quarter =3)b
 on w3.first_dept_id=b.jf_deptid
 ) 
SELECT w4.group_id
      ,w4.group_name
      ,w4.first_dept_id
