...
function pulldownRefresh() {
    setTimeout(function() {
        addData();
        mui('#pullrefresh_4').pullRefresh().endPulldownToRefresh();
        //mui.toast("为你推荐了5篇文章");
    }, 1500);
}
/**
...
function pullupRefresh() {
    setTimeout(function() {
        mui('#pullrefresh_4').pullRefresh().endPullupToRefresh((++count > 2)); //参数为true代表没有更多数据了。
        ...
        for (var i = cells.length, len = i + newCount; i < len; i++) {
            _html='<li class="clearfix">'
		            ...
		            +'</div>'
		            +'<a href="devopsbriefingsure.html" class="right">确认</a>'
