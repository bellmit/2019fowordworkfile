...
function initView() {
	$.ajax({
		url:pathUrl+'/purchaseRevoke.action',
		type:"post",
		data:{
		...
		success:function(data){
			...
			if(json.hasOwnProperty("type2")){
				...
				if(json.ca.includePrice=="1"){
					$("input[name='includePrice'][value=1]").attr("checked",true);
				}else  if(json.ca.includePrice=="0"){
					$("input[name='includePrice'][value=2]").attr("checked",true);
				}
			}
			$("#type").val(json.type);
			...
			var cancelDate=newTime(json.ca.cancelDate);
			...
			$("#cancelReason").val(json.ca.cancelReason);
...
function submits(){
	...
	$.ajax({
		url:pathUrl+'/approvalCancelNPage.action',
		type:"post",
		data:{
			userId:userId,
			id:id,
...
//生成历史审批记录
function list(json){
	if(json.hasOwnProperty("recordApprovalList")){
		$.each(json.recordApprovalList.resultList,function(i,list){
			var attitude="";
			if(list.attitude=="1"){
				attitude="同意";
			}else{
			...
			var html='<li class="clearfix "><span class="left label">节点名称：</span>'+
						'<input type="text" id="" class="f_input right" style="width:75%" value="'+list.taskName+'">'+
					'</li>'+
