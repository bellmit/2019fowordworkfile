...
function initView() {
	$.ajax({
		...
		success:function(data){
			...
			contact(json);
			relationList(json.relation.pageList);
			milestone(json);
			...
			$("#procInstStartUser").val(json.entryMap.applicantName);
			...
			$("#proCode").val(json.relation.proCode);
			var time = dateOfApplicantion.split('-');
			$("#proName").text(json.proName);
			...
			$("#resultDataNames").text(json.entryMap.resultDataNames);
			$("#finalReportFileName").val(json.entryMap.finalReportFileName);
			var projectResult = projectResultValue(json.entryMap.projectResult);
			$("#projectResult").val(projectResult);
...
//获取项目结果
function projectResultValue(data){
	var projectNum = '';
	if(data == '1'){
		projectNum = '签订';
	} else if(data == '2'){
...
//获取里程碑审批状态
function updateApprovalValue(data){
	var approvalStatus = '';
	if(data == '1'){
		approvalStatus = '审批中';
	} else if(data == '2'){
		...
		$.each(json,function(i,list){
			...
			var html='<li class="clearfix "><span class="left label">序号：</span>'+
					...
					'<li class="clearfix "><span class="left label">审批时间：</span> '+
						'<span type="text" id="" class="f_input mui-pull-right" style="padding-right:20px">'+list.taskFinishTime+'</span>'+
					'</li></br>';
...
//生成预算列表
function relationList(json){
	if(json.length > 0){
		$.each(json,function(i,list){
			var html='<li class="clearfix "><span class="left label">编号：</span>'+
						'<input type="text" id="" class="f_input right" style="width:70%" value="'+(i+1)+'">'+
					'</li>'+
					'<li class="clearfix "><span class="left label">费用类型：</span> '+
						'<input type="text" id="" class="f_input right wp60" value="'+list.budgetType+'">'+
					'</li>'+
					'<li class="clearfix "><span class="left label">花费金额(元)：</span> '+
						'<input type="text" id="" class="f_input right wp60" value="'+list.totalFee+'">'+
					'</li>'+
					'<li class="clearfix "><span class="left label">增补原因：</span> '+
						'<input type="text" id="" class="f_input right wp60" value="'+list.additionalReasons+'">'+
					'</li></br>';
...
function milestone(json){
	if(json.hasOwnProperty("pnrData")){
		$.each(json.pnrData,function(i,list){
			...
			var proNodeValues = proNodeValue(list.proNodeStatus);
			var approvalStatus = updateApprovalValue(list.approvalStatus);
			var html='<li class="clearfix "><span class="left label">序号：</span>'+
						'<input type="text" id="" class="f_input right" style="width:70%" value="'+(i+1)+'">'+
					'</li>'+
					...
					'</li>'+
					'<li class="clearfix "><span class="left label">状态：</span>'+
						'<input type="text" id="" class="f_input right wp60" value="'+proNodeValues+'">'+
					'</li>'+
					'<li class="clearfix "><span class="left label">审批状态：</span> '+
						'<input type="text" id="" class="f_input right wp60" value="'+approvalStatus+'">'+
					'</li></br>';
