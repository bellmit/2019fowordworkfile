...
function initView() {
	$.ajax({
		url:pathUrl+'/voidApproval.action',
		type:"post",
		data:{
		...
		success:function(data){
			...
			var jsons = JSON.parse(json.historyList);
			list(json);
			cgFy(json);
...
//生成历史审批记录
function list(json){
	if(json.hasOwnProperty("historyList")){
		var data = JSON.parse(json.historyList);
	...
	$.each(json,function(i,list){
		var html="<li class='clearfix '><span class='left label'>科目分配类别：</span> "+
			...
			"<li class='clearfix '><span class='left label'>一级部门：</span> "+
				"<input type='text' id='cancelDate' class='f_input right wp60' >"+
			"</li>"+
