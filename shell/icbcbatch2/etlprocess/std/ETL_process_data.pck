...
create or replace package body ETL_process_data is
  ...
  begin
    ...
    -- where substr(period_date, 6) not in ('13', '14', '15');
    commit;
    dbms_mview.refresh('t08_bssheet_line_mv','COMPLETE');
    /*
  ...
  begin
    ...
    */
    insert into STD_LAYER.T08_PLSHEET_LINE
      (report_id,
       item_name,
       line_num,
       dept_code,
    ...
    commit ;
    dbms_mview.refresh('t08_plsheet_line_mv','COMPLETE');
    -- where substr(period_date, 6) not in (''13'', ''14'', ''15'')
    /*
    ...
    using (select a.line_num,
                  a.period_date,
                  a.org_code,
                  a.dept_code,
                  sum(b.dept_ptd) sumvalue
             from std_layer.t08_plsheet_line_mv a,
                  std_layer.t08_plsheet_line_mv b
            where a.line_num = b.line_num
              and a.org_code = b.org_code
              and a.dept_code = b.dept_code
              and b.period_date between substr(a.period_date, 1, 4) || '-01' and
                  a.period_date
            group by a.line_num, a.org_code, a.dept_code, a.period_date) suma
    on (a.line_num = suma.line_num and a.org_code = suma.org_code and a.dept_code = suma.dept_code and a.period_date = suma.period_date and a.data_date = query_date)
    when matched then
      update set a.dept_ytd = suma.sumvalue;
  ...
  procedure concat_expdetail_line is
    ...
    /*
    cursor dept_list is
    select column_name from all_tab_columns where owner = 'SRC_LAYER' AND TABLE_NAME = 'EBS_EXPDETAIL_LINE' AND COLUMN_NAME LIKE 'DEPT%' AND COLUMN_NAME LIKE '%PTD%';
    */
  begin
    select data_date
      into query_date
    ...
    /*
    for dept_record in dept_list loop dept_code := substr(dept_record.column_name, 10); execute immediate 'insert into T08_EXPDETAIL_LINE 
            (report_id, item_name,line_num ,etl_date, Data_Date, period_date, dept_code, dept_ptd, dept_ytd, header_id, line_id) 
            select report_id, item_name, line_num, etl_date, Data_Date, period_date , ''' || dept_code || ''' , DEPT_PTD_' || dept_code || ' , DEPT_YTD_' || dept_code || ' , -1 , line_sequence.nextval from src_layer.EBS_EXPDETAIL_LINE';
    */
  
    insert into STD_LAYER.T08_EXPDETAIL_LINE
      (report_id,
       item_name,
       line_num,
       dept_code,
      ...
      select report_id,
        ...
        commit;
      dbms_mview.refresh('t08_expdetail_line_mv','COMPLETE');
    -- where substr(period_date, 6) not in (''13'', ''14'', ''15'')';
    /*
    ...
    */
    ...
    using (select a.line_num,
                  ...
                  a.dept_code,
                  sum(b.dept_ptd) sumvalue
             from std_layer.t08_expdetail_line_mv a,
                  std_layer.t08_expdetail_line_mv b
            where a.line_num = b.line_num
              and a.org_code = b.org_code
              and a.dept_code = b.dept_code
              and b.period_date between substr(a.period_date, 1, 4) || '-01' and
  ...
  begin
    ...
    select sum(h.line_count_num)
      into l_line_count_num
      from src_layer.ebs_bssheet_header h;
    select count(*) into l_count from src_layer.ebs_bssheet_line l;
    --判断
    if l_line_count_num = l_count then
  ...
  begin
    ...
    select sum(h.line_count_num)
      into l_line_count_num
      from src_layer.ebs_plsheet_header h;
    select count(*) into l_count from src_layer.ebs_plsheet_line l;
  ...
  begin
    ...
    select sum(h.line_count_num)
      into l_line_count_num
      from src_layer.ebs_expdetail_header h;
    select count(*) into l_count from src_layer.ebs_expdetail_line l;
