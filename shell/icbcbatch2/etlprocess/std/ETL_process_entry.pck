...
create or replace package body ETL_process_entry is
  --校验
  procedure ebs_src_std is
    query_date date;
  
    l_line_count1  number;
    l_count1       number;
    l_line_count2  number;
    l_count2       number;
    l_line_count3  number;
    l_count3       number;
    l_line_count4  number;
    l_count4       number;
    l_line_count5  number;
    l_count5       number;
    l_line_count6  number;
    l_count6       number;
    l_line_count7  number;
    l_count7       number;
    l_line_count8  number;
    l_count8       number;
    l_line_count9  number;
    l_count9       number;
    l_line_count10 number;
    l_count10      number;
  begin
    ...
    --账套
    select h.line_count_num
      into l_line_count1
      from src_layer.ebs_data_header h
     where h.report_flag = 'gl_ledgers';
    select count(*) into l_count1 from src_layer.ebs_gl_ledgers;
    --CCID
    select sum(h.line_count_num)
      into l_line_count2
      from src_layer.ebs_data_header h
     where h.report_flag = 'gl_code_combinations';
    select count(*) into l_count2 from src_layer.ebs_gl_code_combinations;
    --gl_balance
    select sum(h.line_count_num)
      into l_line_count3
      from src_layer.ebs_data_header h
     where h.report_flag = 'gl_balances';
    select count(*) into l_count3 from src_layer.ebs_gl_balances;
    --弹性域
    select sum(h.line_count_num)
      into l_line_count4
      from src_layer.ebs_data_header h
     where h.report_flag = 'fnd_id_flex_segments';
    select count(*) into l_count4 from src_layer.ebs_fnd_id_flex_segments;
    --值集set
    select sum(h.line_count_num)
      into l_line_count5
      from src_layer.ebs_data_header h
     where h.report_flag = 'fnd_flex_value_sets';
    select count(*) into l_count5 from src_layer.ebs_fnd_flex_value_sets;
    --值集value
    select sum(h.line_count_num)
      into l_line_count6
      from src_layer.ebs_data_header h
     where h.report_flag = 'fnd_flex_values';
    select count(*) into l_count6 from src_layer.ebs_fnd_flex_values;
    --发票行
    select sum(h.line_count_num)
      into l_line_count7
      from src_layer.ebs_data_header h
     where h.report_flag = 'ap_invoice_lines_all';
    select count(*) into l_count7 from src_layer.ebs_ap_invoice_lines_all;
    --发票分配行
    select sum(h.line_count_num)
      into l_line_count8
      from src_layer.ebs_data_header h
     where h.report_flag = 'ap_invoice_distributions_all';
    select count(*)
      into l_count8
    ...
    select sum(h.line_count_num)
      into l_line_count9
      from src_layer.ebs_data_header h
     where h.report_flag = 'ap_invoices_all';
    select count(*) into l_count9 from src_layer.ebs_ap_invoices_all;
    --供应商
    select sum(h.line_count_num)
      into l_line_count10
      from src_layer.ebs_data_header h
     where h.report_flag = 'ap_suppliers';
    select count(*) into l_count10 from src_layer.ebs_ap_suppliers;
  
    if l_line_count1 = l_count1 then
      update_gl_ledgers;
    else
    ...
    if l_line_count2 = l_count2 then
      update_gl_code_combinations;
    else
    ...
    if l_line_count3 = l_count3 then
      update_gl_balances;
    else
      update_log('SRC_LAYER.EBS_GL_BALANCE',
    ...
    if l_line_count4 = l_count4 then
      update_fnd_id_flex_segments;
    else
    ...
    if l_line_count5 = l_count5 then
      update_fnd_flex_value_sets;
    else
      update_log('SRC_LAYER.EBS_FND_FLEX_VALUES_SETS',
    ...
    if l_line_count6 = l_count6 then
      update_fnd_flex_values;
    else
    ...
    if l_line_count7 = l_count7 then
      update_ap_invoice_lines_all;
    else
    ...
    if l_line_count8 = l_count8 then
      update_ap_invoice_distributions_all;
    else
    ...
    if l_line_count9 = l_count9 then
      update_ap_invoices_all;
    else
    ...
    if l_line_count10 = l_count10 then
      update_ap_suppliers;
    else
  ...
  */
  ...
  begin
    ...
    on (nvl(HIS.ledger_id, 0) = nvl(DIF.ledger_id, 0) and nvl(HIS.code_combination_id, 0) = nvl(DIF.code_combination_id, 0) and nvl(his.currency_code, 0) = nvl(dif.currency_code, 0) and nvl(his.period_name, 0) = nvl(dif.period_name, 0) and nvl(his.actual_flag, 0) = nvl(dif.actual_flag, 0) and nvl(his.encumbrance_type_id, 0) = nvl(dif.encumbrance_type_id, 0) and nvl(his.translated_flag, 0) = nvl(dif.translated_flag, 0) and nvl(his.budget_version_id, 0) = nvl(dif.budget_version_id, 0))
    when matched then
      update set HIS.end_date = query_date;
    commit;
  ...
  begin
    ...
    on (HIS.application_id = DIF.application_id and his.id_flex_code = dif.id_flex_code and his.id_flex_num = dif.id_flex_num)
    when matched then
      update set HIS.end_date = query_date;
